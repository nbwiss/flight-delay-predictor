...
y_canc = df['target_cancelled'].values.astype(np.int8)
y_del  = df['target_delayed'].values.astype(np.int8)
y_mins = df['target_delay_minutes'].values.astype(np.float32)
...
# %% Build Tree Feature Matrix (OrdinalEncoder — dense, ~50 features)
print("Building tree feature matrix (no leakage)...")
t0 = time.time()
...
cat_tr_ord = ord_enc.fit_transform(df_tr[CAT_COLS])
cat_te_ord = ord_enc.transform(df_te[CAT_COLS])

FEATURE_NAMES = NUM_COLS + CAT_COLS
N_NUM = len(NUM_COLS)  # boundary index: X[:, :N_NUM] = scaled numerics

X_train = np.hstack([num_tr, cat_tr_ord]).astype(np.float32)
X_test  = np.hstack([num_te, cat_te_ord]).astype(np.float32)

print(f"X_train: {X_train.shape}  |  X_test: {X_test.shape}  |  built in {time.time()-t0:.1f}s")

del df_tr, df_te, num_tr, num_te, cat_tr_ord, cat_te_ord
gc.collect()

# Non-cancelled subsets for Models B & C
nc_tr = y_canc_tr == 0
nc_te = y_canc_te == 0

X_tr_b, y_tr_b = X_train[nc_tr], y_del_tr[nc_tr]
X_te_b, y_te_b = X_test[nc_te],  y_del_te[nc_te]
X_tr_c, y_tr_c = X_train[nc_tr], y_mins_tr[nc_tr]
X_te_c, y_te_c = X_test[nc_te],  y_mins_te[nc_te]

spw_canc  = (y_canc_tr == 0).sum() / max((y_canc_tr == 1).sum(), 1)
spw_delay = (y_tr_b == 0).sum() / max((y_tr_b == 1).sum(), 1)

print(f"Model A  train: {X_train.shape[0]:,}  test: {X_test.shape[0]:,}  pos: {y_canc_tr.mean():.4f}")
...
SAVE_DIR = '/content/drive/MyDrive/flight_models'
...
print("="*60)
results_a = run_all("ModelA_Cancel", configs_a, X_train, y_canc_tr, X_test, y_canc_te, task='clf')
...
for label, results, Xte, best_algo in [
    ("ModelA_Cancel", results_a, X_test, best_a_algo),
...
joblib.dump(ord_enc, os.path.join(SAVE_DIR, 'ord_encoder.joblib'))
joblib.dump(medians, os.path.join(SAVE_DIR, 'medians.joblib'))
